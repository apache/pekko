...
import pekko.persistence.query.typed.internal.EventsBySliceFirehose.SlowConsumerException
...
object EventsBySliceFirehoseSpec {
  // real PersistenceId is in pekko-persistence-typed, and no dependency to that from here
  ...
  events-by-slice-firehose-with-deduplication = $${pekko.persistence.query.events-by-slice-firehose}
