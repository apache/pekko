...
object Replicator {
  ...
  /**
   * The `Behavior` for the `Replicator` actor.
   * It will use the given underlying [[pekko.cluster.ddata.Replicator]]
   */
  def behavior(settings: dd.ReplicatorSettings, underlyingReplicator: pekko.actor.ActorRef): Behavior[Command] =
  ...
  sealed trait ReadConsistency {
    ...
    /** INTERNAL API */
    @InternalApi private[pekko] def toClassic: dd.Replicator.ReadConsistency
  }
  ...
  sealed trait WriteConsistency {
    ...
    /** INTERNAL API */
    @InternalApi private[pekko] def toClassic: dd.Replicator.WriteConsistency
  }
  case object WriteLocal extends WriteConsistency {
    override def timeout: Duration = Duration.ZERO

    /** INTERNAL API */
    @InternalApi private[pekko] override def toClassic = dd.Replicator.WriteLocal
  }
   ...
   */
  @InternalApi private[pekko] case object FlushChanges extends Command
