 ...
 */
@InternalApi
private[pekko] object VersionVector {
  ...
  /** INTERNAL API */
  @InternalApi private[pekko] def apply(versions: List[(String, Long)]): VersionVector =
    if (versions.isEmpty) empty
 ...
 * }}}
 *
 * Based on `org.apache.pekko.cluster.ddata.VersionVector`.
 *
 * This class is immutable, i.e. "modifying" methods return a new instance.
 */
@InternalApi
private[pekko] sealed abstract class VersionVector {
   ...
   * INTERNAL API
   */
  @InternalApi private[pekko] def contains(key: String): Boolean
   ...
   * INTERNAL API
   */
  @InternalApi private[pekko] def versionsIterator: Iterator[(String, Long)]
 ...
 * INTERNAL API
 */
@InternalApi private[pekko] final case class OneVersionVector private[pekko] (key: String, version: Long)
  ...
  /** INTERNAL API */
  @InternalApi private[pekko] override def contains(k: String): Boolean =
    k == key
 ...
 * INTERNAL API
 */
@InternalApi private[pekko] final case class ManyVersionVector(versions: TreeMap[String, Long]) extends VersionVector {
  import VersionVector.Timestamp
  ...
  /** INTERNAL API */
  @InternalApi private[pekko] override def contains(key: String): Boolean =
