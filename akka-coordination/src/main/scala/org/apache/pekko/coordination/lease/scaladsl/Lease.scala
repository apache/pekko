...
abstract class Lease(val settings: LeaseSettings) {
   ...
   * acquired, e.g. because some other owner is holding it. It's completed with [[pekko.coordination.lease.LeaseException]]
   * failure if it might not have been able to acquire the lease, e.g. communication timeout
   * with the lease resource.
   *
   * The lease will be held by the [[pekko.coordination.lease.LeaseSettings.ownerName]] until it is released
