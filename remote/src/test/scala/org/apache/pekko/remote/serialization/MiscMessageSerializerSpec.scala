...
class MiscMessageSerializerSpec extends PekkoSpec(MiscMessageSerializerSpec.testConfig) {
  ...
  "MiscMessageSerializer" must {
    Seq(
      ...
      "Done" -> Done,
      "NotUsed" -> NotUsed,
      "Address" -> Address("pekko", "system", "host", 1337),
      "UniqueAddress" -> pekko.remote.UniqueAddress(Address("pekko", "system", "host", 1337), 82751),
      "LocalScope" -> LocalScope,
      "RemoteScope" -> RemoteScope(Address("pekko", "system", "localhost", 2525)),
      ...
      "RemoteRouterConfig" -> RemoteRouterConfig(
        local = RandomPool(25),
        nodes = List(Address("pekko", "system", "localhost", 2525))),
