...
final class SubFlow[In, Out, Mat](
  ...
  def statefulMap[S, T](
      ...
      onComplete: function.Function[S, Optional[T]]): javadsl.SubFlow[In, T, Mat] =
    new SubFlow(
   ...
   * @since 1.3.0
   */
  def doOnFirst(f: function.Procedure[Out]): SubFlow[In, Out, Mat] = new javadsl.SubFlow(delegate.doOnFirst(f(_)))
   ...
   *
   * See also [[SubFlow.limit]], [[SubFlow.limitWeighted]], [[SubFlow.takeWhile]]
   */
   ...
   * that will then not be processed downstream of this step.
   *
   * The stream will be completed without producing any elements if predicate is true for
   * the first stream element.
   *
   * '''Emits when''' the predicate is false or the first time the predicate is true
   *
   ...
   *  @since 1.1.0
   */
  def onErrorComplete(predicate: function.Predicate[? >: Throwable]): SubFlow[In, Out, Mat] =
  ...
  def onErrorContinue[T <: Throwable](clazz: Class[T],
    ...
    new SubFlow(delegate.onErrorContinue(clazz.isInstance)(errorConsumer.apply))
  ...
  def prefixAndTail(n: Int): SubFlow[
    ...
    new SubFlow(delegate.prefixAndTail(n).map { case (taken, tail) => pekko.japi.Pair(taken.asJava, tail.asJava) })
