...
object MergeHub {
  ...
  private final class DrainingControlImpl(c: pekko.stream.scaladsl.MergeHub.DrainingControl) extends DrainingControl {
   ...
   * @param clazz Type of elements this hub emits and consumes
   */
  def withDraining[T](clazz: Class[T]): Source[T, pekko.japi.Pair[Sink[T, NotUsed], DrainingControl]] =
...
object BroadcastHub {
  ...
  def of[T](@unused clazz: Class[T], startAfterNrOfConsumers: Int, bufferSize: Int): Sink[T, Source[T, NotUsed]] = {
    pekko.stream.scaladsl.BroadcastHub.sink[T](startAfterNrOfConsumers, bufferSize).mapMaterializedValue(_.asJava).asJava
  }
  ...
  def of[T](clazz: Class[T]): Sink[T, Source[T, NotUsed]] =
    of(clazz, pekko.stream.scaladsl.BroadcastHub.defaultBufferSize)
