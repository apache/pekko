...
final class Flow[In, Out, Mat](delegate: scaladsl.Flow[In, Out, Mat]) extends Graph[FlowShape[In, Out], Mat] {
  ...
  def preMaterialize(materializer: Materializer): pekko.japi.Pair[Mat @uncheckedVariance, Flow[In, Out, NotUsed]] = {
    ...
    pekko.japi.Pair(mat, flow.asJava)
  }
  ...
  def runWith[T, U](
      ...
      systemProvider: ClassicActorSystemProvider): pekko.japi.Pair[T, U] = {
  ...
  def runWith[T, U](
      source: Graph[SourceShape[In], T],
      sink: Graph[SinkShape[Out], U],
      materializer: Materializer): pekko.japi.Pair[T, U] = {
   ...
   * @since 1.3.0
   */
  def doOnFirst(f: function.Procedure[Out]): javadsl.Flow[In, Out, Mat] = new Flow(delegate.doOnFirst(f(_)))
   ...
   * @since 1.3.0
   */
  def doOnCancel(f: function.Procedure2[? >: Throwable, java.lang.Boolean]): javadsl.Flow[In, Out, Mat] =
    new Flow(delegate.doOnCancel((ex: Throwable, wasCancelledNormally: Boolean) => f(ex, wasCancelledNormally)))
   ...
   * @since 1.2.0
   */
  def dropRepeated(): javadsl.Flow[In, Out, Mat] =
    new Flow(delegate.dropRepeated())
   ...
   * @since 1.2.0
   */
  def dropRepeated(p: function.Function2[Out, Out, Boolean]): javadsl.Flow[In, Out, Mat] =
    new Flow(delegate.dropRepeated(p.apply))
   ...
   * '''Cancels when''' downstream cancels
   */
  def recover(clazz: Class[? <: Throwable], creator: function.Creator[Out]): javadsl.Flow[In, Out, Mat] =
    recover {
   ...
   * @since 1.3.0
   */
  def recover(clazz: Class[? <: Throwable], fallbackValue: Out): javadsl.Flow[In, Out, Mat] =
    recover {
   ...
   * @since 1.3.0
   */
  def recover(p: function.Predicate[? >: Throwable], creator: function.Creator[Out]): javadsl.Flow[In, Out, Mat] =
    recover {
   ...
   * @since 1.3.0
   */
  def recover(p: function.Predicate[? >: Throwable], fallbackValue: Out): javadsl.Flow[In, Out, Mat] =
    recover {
   ...
   *  @since 1.1.0
   */
  def onErrorComplete(predicate: function.Predicate[? >: Throwable]): javadsl.Flow[In, Out, Mat] =
   ...
   * @since 1.3.0
   */
  def onErrorContinue(errorConsumer: function.Procedure[? >: Throwable]): javadsl.Flow[In, Out, Mat] =
  ...
  def onErrorContinue[T <: Throwable](clazz: Class[T],
      errorConsumer: function.Procedure[? >: Throwable]): javadsl.Flow[In, Out, Mat] =
   ...
   * @since 1.3.0
   */
  def onErrorContinue[T <: Throwable](predicate: function.Predicate[? >: Throwable],
      errorConsumer: function.Procedure[? >: Throwable]): javadsl.Flow[In, Out, Mat] =
   ...
   * @since 1.3.0
   */
  def onErrorResume[T >: Out](fallback: function.Function[? >: Throwable, ? <: Graph[SourceShape[T], NotUsed]])
  ...
  def onErrorResume[T >: Out](
      clazz: Class[? <: Throwable],
      fallback: function.Function[? >: Throwable, ? <: Graph[SourceShape[T], NotUsed]])
      : javadsl.Flow[In, T, Mat] = new Flow(delegate.recoverWith {
   ...
   */
  def prefixAndTail(n: Int): javadsl.Flow[In, pekko.japi.Pair[java.util.List[Out], javadsl.Source[Out, NotUsed]], Mat] =
    new Flow(delegate.prefixAndTail(n).map { case (taken, tail) => pekko.japi.Pair(taken.asJava, tail.asJava) })
  ...
  def flatMapConcat[T, M](
      parallelism: Int,
      f: function.Function[Out, ? <: Graph[SourceShape[T], M]]): Flow[In, T, Mat] =
   ...
   * @since 1.2.0
   */
  def switchMap[T, M](f: function.Function[Out, ? <: Graph[SourceShape[T], M]]): Flow[In, T, Mat] =
   ...
   */
  def interleaveAll(
      @Nullable those: java.util.List[? <: Graph[SourceShape[Out], ? <: Any]],
      segmentSize: Int,
