 ...
 *
 * The "tree-ness" is expressed by explicit [[org.apache.pekko.stream.impl.Concat]] nodes that express that two traversals
 ...
 * command types:
 *
 *  * [[org.apache.pekko.stream.impl.MaterializeAtomic]]: An atomic module needs to be materialized. This node also contains
 ...
 *    * [[org.apache.pekko.stream.impl.Transform]] take the top of the stack, transform it with the provided function and put
 *      the result back on the top of the stack
 *    * [[org.apache.pekko.stream.impl.Compose]] take the top two values of the stack, invoke the provided function with these
 *      values as arguments, then put the calculated value on the top of the stack
 *    * Materialized values of atomic operators when visiting a [[org.apache.pekko.stream.impl.MaterializeAtomic]] must be
 ...
 *    * [[org.apache.pekko.stream.impl.PushAttributes]] combines the attributes on the top of the stack with the given ones and
 ...
 *    extension point where "plugins" ([[org.apache.pekko.stream.impl.PhaseIsland]]) can be used to specially handle subgraphs.
 ...
 *     * [[org.apache.pekko.stream.impl.EnterIsland]] instructs the materializer that the following commands will belong to
 *       the materialization of a new island (a subgraph). The [[org.apache.pekko.stream.impl.IslandTag]] signals to
 *       the materializer which [[org.apache.pekko.stream.impl.PhaseIsland]] should be used to turn operators of this island into
 *       executable entities.
 *     * [[org.apache.pekko.stream.impl.ExitIsland]] instructs the materializer that the current island is done and the parent
 ...
 *  (Please note that all [[org.apache.pekko.stream.impl.StreamLayout.AtomicModule]]s maintain a stable order of their ports, so
 ...
 *
 *   * offset of a module: The offset of an [[org.apache.pekko.stream.impl.StreamLayout.AtomicModule]] is defined as the value of
 *     the input port counter when visiting the [[org.apache.pekko.stream.impl.MaterializeAtomic]] node to materialize that module.
 ...
 *  a [[org.apache.pekko.stream.impl.MaterializeAtomic]] node, it contains an Array that maps ports to a relative offset. To
 ...
 *
 *  The simpler case is the [[org.apache.pekko.stream.impl.LinearTraversalBuilder]]. This builder only allows building linear
 ...
 *  is not ambiguous (see previous section). As a result, the [[org.apache.pekko.stream.impl.LinearTraversalBuilder]] does not
 ...
 *  additional state we need to track. On the other hand, we cannot build a [[org.apache.pekko.stream.impl.MaterializeAtomic]]
 ...
 *  information to be able to assign output ports, build the outToSlots array and finally the [[org.apache.pekko.stream.impl.MaterializeAtomic]]
 *  node. The consequence of this that a [[org.apache.pekko.stream.impl.Traversal]] can be constructed as soon as all output ports
 ...
 *  [[org.apache.pekko.stream.impl.CompletedTraversalBuilder]]. This builder type simply contains the completed traversal plus
 ...
 *
 *  The most generic builder is [[org.apache.pekko.stream.impl.CompositeTraversalBuilder]]. There are two main considerations
 ...
 *  as a [[org.apache.pekko.stream.impl.BuilderKey]] which is just a placeholder where the traversal of the submodule will be
 *  stitched in. This [[org.apache.pekko.stream.impl.BuilderKey]] is also a key to a map which contains the evolving builder.
 ...
 *  using the submodule traversals referred to by [[org.apache.pekko.stream.impl.BuilderKey]]. Note that at this point all the
 *  submodules are [[org.apache.pekko.stream.impl.CompletedTraversalBuilder]]s because there are no unwired outputs and hence the
 *  Traversal can be assembled. As the builder evolves over time, more and more of its [[org.apache.pekko.stream.impl.BuilderKey]]s
 *  will refer to [[org.apache.pekko.stream.impl.CompletedTraversalBuilder]]s, shedding much of the temporary data structures.
 *
 *  Refer to [[org.apache.pekko.stream.impl.CompositeTraversalBuilder]] for more details.
 *
 *  The [[org.apache.pekko.stream.impl.LinearTraversalBuilder]] is a much simpler beast. For efficiency, it tries to work as much
 *  as possible directly on the [[org.apache.pekko.stream.impl.Traversal]] avoiding auxiliary structures. The two main considerations
 *  for this builder are:
 *
 *   * [[org.apache.pekko.stream.scaladsl.Source]] and [[org.apache.pekko.stream.scaladsl.Flow]] contain an unwired output port. Yet, we would
 *     like to build the traversal directly as much as possible, even though the builder is not yet completed
 *   * [[org.apache.pekko.stream.impl.CompositeTraversalBuilder]]s might be included in a linear chain. These cannot provide a
 ...
 *     of builder only contains the [[org.apache.pekko.stream.impl.Traversal]] and only a couple of extra fields.
 ...
 *  later, we fix it by the helper method [[org.apache.pekko.stream.impl.Traversal.rewireFirstTo]] which tears down the traversal
 ...
 *  this composite and delay construction of its part of the traversal. For details see [[org.apache.pekko.stream.impl.LinearTraversalBuilder]]
 ...
 *  == Materialization ==
 *
 *  Materialzation is orchestrated by the [[org.apache.pekko.stream.impl.PhasedFusingActorMaterializer]]. It basically decodes the
 ...
 *  to [[org.apache.pekko.stream.impl.PhaseIsland]]s. For example a default fused island will be actually wired by
 ...
 *  slots between [[org.apache.pekko.stream.impl.EnterIsland]] or [[org.apache.pekko.stream.impl.ExitIsland]] tags (in any combination). When
 ...
 *  the information needed, i.e. we visit the target in port. The [[org.apache.pekko.stream.impl.PhasedFusingActorMaterializer]]
 ...
 *
 *  Remember, the [[org.apache.pekko.stream.impl.LinearTraversalBuilder]] builds its [[org.apache.pekko.stream.impl.Traversal]] in backwards
 ...
 *  as the [[org.apache.pekko.stream.impl.IslandTracking]] class tracks the offset of the current segment. If the target input
 *  slot is larger or equal than this offset, and the wiring is backwards, then the wiring is strictly local to the
 *  island. The materializer will simply delegate to the [[org.apache.pekko.stream.impl.PhaseIsland]] to do the internal wiring.
 *  Since we know the offset of the segment in the local space of this island, calculating the local slot for the
 *  [[org.apache.pekko.stream.impl.PhaseIsland]] is simple. (This is fully documented with diagrams
 *  in [[org.apache.pekko.stream.impl.IslandTracking]])
 *
 ...
 *    |----Island1-----|----Island2(enclosed)----|-----Island1-----|
 *
 *  In this case, the target slot is in another, but already visited segment. The [[org.apache.pekko.stream.impl.IslandTracking]]
 *  class needs to first find the segment in which the target slot is. Since each segment keeps a reference to its
 *  [[org.apache.pekko.stream.impl.PhaseIsland]] instance that handles the internal wiring a simple reference equality check
 ...
 *  and call the appropriate callback on the [[org.apache.pekko.stream.impl.PhaseIsland]]. (This is fully documented with diagrams
 *  in [[org.apache.pekko.stream.impl.IslandTracking]])
 *
 ...
 *  equality of the current [[org.apache.pekko.stream.impl.PhaseIsland]] with that of the target segment (we have already found
 ...
 *  ask the output [[org.apache.pekko.stream.impl.PhaseIsland]] to provide a Publisher and then we ask the target island to
 *  take this Publisher.
 *
