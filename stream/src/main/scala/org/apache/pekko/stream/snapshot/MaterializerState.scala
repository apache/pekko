...
object MaterializerState {
  ...
  /** INTERNAL API */
  @InternalApi
  private[pekko] def requestFromChild(child: ActorRef): Future[StreamSnapshot] = {
 ...
 */
@InternalApi
final private[pekko] case class StreamSnapshotImpl(
    self: ActorPath,
 ...
 */
@InternalApi
private[pekko] final case class LogicSnapshotImpl(index: Int, label: String, attributes: Attributes)
 ...
 */
@InternalApi
private[pekko] final case class ConnectionSnapshotImpl(
    id: Int,
    in: LogicSnapshot,
    out: LogicSnapshot,
