...
object Behaviors {
   ...
   */
  def setup[T](factory: pekko.japi.function.Function[ActorContext[T], Behavior[T]]): Behavior[T] =
   ...
   * the same interceptor (defined by the [[pekko.actor.typed.BehaviorInterceptor#isSame]] method) only the innermost interceptor
