...
object ConsumerController {
  ...
  final case class SequencedMessage[A](
      ...
      first: Boolean,
      ack: Boolean)(
      @InternalApi private[pekko] val producerController: ActorRef[ProducerControllerImpl.InternalCommand])
      extends Command[A]
      with DeliverySerializable
      with DeadLetterSuppression {

    /** INTERNAL API */
    @InternalApi private[pekko] def asFirst: SequencedMessage[A] =
   ...
   * [[pekko.actor.typed.receptionist.Receptionist]] with the given `serviceKey`, and the
