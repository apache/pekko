 ...
 *
 * In Pekko, the first capability is accessed by using the `!` or `tell` method
...
trait ActorContext[T] extends TypedActorContext[T] with ClassicActorContextProvider {
   ...
   */
  def asJava: pekko.actor.typed.javadsl.ActorContext[T]
  ...
  /**
   * Create a child Actor from the given [[pekko.actor.typed.Behavior]] under a randomly chosen name.
  ...
  /**
   * Delegate message and signal's execution by given [[pekko.actor.typed.Behavior]]
   ...
   * by invoking [[pekko.actor.Cancellable#cancel]] on the returned
   * handle.
   *
   * This method is thread-safe and can be called from other threads than the ordinary
   * actor message processing thread, such as [[scala.concurrent.Future]] callbacks.
   */
  def scheduleOnce[U](delay: FiniteDuration, target: ActorRef[U], msg: U): pekko.actor.Cancellable
   ...
   * state of the "parent".
   */
  @InternalApi private[pekko] def spawnMessageAdapter[U](f: U => T, name: String): ActorRef[U]

  /**
   * INTERNAL API: See `spawnMessageAdapter` with name parameter
   */
  @InternalApi private[pekko] def spawnMessageAdapter[U](f: U => T): ActorRef[U]
   ...
   */
  @InternalApi
  private[pekko] def onUnhandled(msg: T): Unit

  /**
   * INTERNAL API
   */
  @InternalApi
  private[pekko] def currentBehavior: Behavior[T]

  /**
   * INTERNAL API
   */
  @InternalApi
  private[pekko] def hasTimer: Boolean

  /**
   * INTERNAL API
   */
  @InternalApi
  private[pekko] def cancelAllTimers(): Unit

  /**
   * INTERNAL API
   */
  @InternalApi
  private[pekko] def clearMdc(): Unit
