 ...
 */
@SerialVersionUID(1L)
private[pekko] final case class SerializedActorRef[T] private (address: String) {
  import pekko.actor.typed.scaladsl.adapter._
  import pekko.serialization.JavaSerializer.currentSystem
  ...
  def readResolve(): AnyRef = currentSystem.value match {
    case null =>
      throw new IllegalStateException(
        ...
        " Use 'org.apache.pekko.serialization.Serialization.currentSystem.withValue(system) { ... }'")
    case someSystem =>
