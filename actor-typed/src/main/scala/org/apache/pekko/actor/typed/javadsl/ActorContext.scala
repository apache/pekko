...
trait ActorContext[T] extends TypedActorContext[T] with ClassicActorContextProvider {
   ...
   */
  def asScala: pekko.actor.typed.scaladsl.ActorContext[T]
   ...
   * actor message processing thread, such as [[java.util.concurrent.CompletionStage]] callbacks.
   */
  def scheduleOnce[U](delay: Duration, target: ActorRef[U], msg: U): pekko.actor.Cancellable
   ...
   * than the ordinary actor message processing thread, such as [[java.util.concurrent.CompletionStage]] callbacks.
   */
  def messageAdapter[U](messageClass: Class[U], f: pekko.japi.function.Function[U, T]): ActorRef[U]
   ...
   *                        [[pekko.actor.Status.Failure]] as response. The returned message of type `T` is then
