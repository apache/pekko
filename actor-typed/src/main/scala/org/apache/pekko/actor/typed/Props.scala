 ...
 */
@DoNotInherit
abstract class Props private[pekko] () extends Product with Serializable {
   ...
   */
  @InternalApi
  private[pekko] def withNext(next: Props): Props
   ...
   */
  @InternalApi
  private[pekko] def allOf[T <: Props: ClassTag]: List[Props] = {
