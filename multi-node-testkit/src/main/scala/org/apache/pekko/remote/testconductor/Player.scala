 ...
 * INTERNAL API.
 */
private[pekko] object PlayerHandler {
  case class Reconnected(connection: RemoteConnection)
}
...
private[pekko] class PlayerHandler(
  ...
  var connection: RemoteConnection = reconnect()
  ...
  override def exceptionCaught(ctx: ChannelHandlerContext, cause: Throwable): Unit = {
    ...
    cause match {
      case _: ConnectException if reconnects > 0 =>
        reconnects -= 1
        scheduler.scheduleOnce(nextAttempt.timeLeft) {
          val remoteConnection = reconnect()
          remoteConnection.channel.pipeline().fireUserEventTriggered(PlayerHandler.Reconnected(remoteConnection))
        }
  ...
  override def userEventTriggered(ctx: ChannelHandlerContext, evt: Any): Unit = evt match {
    case PlayerHandler.Reconnected(c) => connection = c
  ...
  override def channelInactive(ctx: ChannelHandlerContext): Unit = {
    ...
    fsm ! PoisonPill
    executor.execute(() => connection.shutdown()) // Must be shutdown outside of the Netty IO pool
  }
