 ...
 *
 * <p>Andriy Plokhotnuyk (https://github.com/plokhotnyuk) -
 ...
 *
 * <p>Dmitriy Vyukov's non-intrusive MPSC queue: -
...
public abstract class AbstractBoundedNodeQueue<T> {
  ...
  protected final Node<T> peekNode() {
    for (; ; ) {
      ...
      if (next != null || getEnq() == deq) return next;
    }
  }
  ...
  // Possible TODO — impl. could be switched to addNode(new Node(value)) if we want to allocate even
  // if full already
  ...
  public final int size() {
    ...
    for (; ; ) {
      ...
      if (deqCountAfter == deqCountBefore) return enqCount - deqCountAfter;
    }
  }
  ...
  private static final long enqOffset, deqOffset;

  static {
    try {
      enqOffset =
          Unsafe.instance.objectFieldOffset(
              AbstractBoundedNodeQueue.class.getDeclaredField("_enqDoNotCallMeDirectly"));
      deqOffset =
          Unsafe.instance.objectFieldOffset(
              AbstractBoundedNodeQueue.class.getDeclaredField("_deqDoNotCallMeDirectly"));
    } catch (Throwable t) {
