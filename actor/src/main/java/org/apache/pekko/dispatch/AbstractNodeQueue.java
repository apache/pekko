 ...
 *
 * <p>This queue could be wait-free (i.e. without the spinning loops in peekNode and pollNode) if it
 * were permitted to return null while the queue is not quite empty anymore but the enqueued element
 * is not yet visible. This would break actor scheduling, though.
 */
...
public abstract class AbstractNodeQueue<T> extends AtomicReference<AbstractNodeQueue.Node<T>> {
   ...
   *
   * <p>Use this method only from the consumer thread!
   *
   * <p>!!! There is a copy of this code in pollNode() !!!
   *
   ...
   *
   * <p>This method can be used from any thread.
   *
  ...
  /**
   * This method returns an upper bound on the queue size at the time it starts executing. It may
   * spuriously return smaller values (including zero) if the consumer pulls items out concurrently.
   *
   * <p>This method can be used from any thread.
   *
    ...
    try {
      tailOffset =
          Unsafe.instance.objectFieldOffset(
              AbstractNodeQueue.class.getDeclaredField("_tailDoNotCallMeDirectly"));
    } catch (Throwable t) {
  ...
  public static class Node<T> {
    ...
    static {
      try {
        nextOffset =
            Unsafe.instance.objectFieldOffset(
                Node.class.getDeclaredField("_nextDoNotCallMeDirectly"));
      } catch (Throwable t) {
        throw new ExceptionInInitializerError(t);
      }
    }
