...
object FSM {
  ...
  /** INTERNAL API */
  @InternalApi
  private[pekko] sealed trait TimerMode {
    def repeat: Boolean
  }

  /** INTERNAL API */
  @InternalApi
  private[pekko] case object FixedRateMode extends TimerMode {
    override def repeat: Boolean = true
  }

  /** INTERNAL API */
  @InternalApi
  private[pekko] case object FixedDelayMode extends TimerMode {
    override def repeat: Boolean = true
  }

  /** INTERNAL API */
  @InternalApi
  private[pekko] case object SingleMode extends TimerMode {
   ...
   */
  private[pekko] class SilentState[S, D](
      stateName: S,
      stateData: D,
     ...
     * INTERNAL API
     */
    private[pekko] override def notifies: Boolean = false
  ...
  class State[S, D](
     ...
     * INTERNAL API
     */
    private[pekko] def notifies: Boolean = true
...
trait FSM[S, D] extends Actor with Listeners with ActorLogging {
   ...
   */
  private[pekko] final def isStateTimerActive = timeoutFuture.isDefined
   ...
   * last call within the constructor, or [[pekko.actor.Actor#preStart]] and
...
trait LoggingFSM[S, D] extends FSM[S, D] { this: Actor =>
  ...
  private[pekko] override val debugEvent = context.system.settings.FsmDebugEvent
  ...
  private[pekko] abstract override def processEvent(event: Event, source: AnyRef): Unit = {
    if (debugEvent) {
