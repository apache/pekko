...
object CoordinatedShutdown extends ExtensionId[CoordinatedShutdown] with ExtensionIdProvider {
  ...
  /** INTERNAL API */
  @InternalApi
  private[pekko] def init(system: ExtendedActorSystem, conf: Config, coord: CoordinatedShutdown): Unit = {
   ...
   * INTERNAL API
   */
  private[pekko] final case class Phase(
   ...
   */
  private[pekko] def topologicalSort(phases: Map[String, Phase]): List[String] = {
 ...
 */
@InternalApi
private[pekko] object JVMShutdownHooks extends JVMShutdownHooks {
...
final class CoordinatedShutdown private[pekko] (
    system: ExtendedActorSystem,
  ...
  /** INTERNAL API */
  private[pekko] val log = Logging(system, classOf[CoordinatedShutdown])
  private val knownPhases = phases.keySet ++ phases.values.flatMap(_.dependsOn)

  /** INTERNAL API */
  private[pekko] val orderedPhases = CoordinatedShutdown.topologicalSort(phases)
   ...
   * INTERNAL API
   */
  private[pekko] lazy val terminationWatcher =
    system.systemActorOf(CoordinatedShutdownTerminationWatcher.props, "coordinatedShutdownTerminationWatcher")

  /**
   * INTERNAL API
   */
  private[pekko] def jvmHooksLatch: CountDownLatch = _jvmHooksLatch.get
...
/** INTERNAL API */
@InternalApi
private[pekko] object CoordinatedShutdownTerminationWatcher {
...
/** INTERNAL API */
@InternalApi
private[pekko] class CoordinatedShutdownTerminationWatcher extends Actor with Timers {
