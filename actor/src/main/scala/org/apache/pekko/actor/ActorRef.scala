 ...
 * [[pekko.actor.ActorRefFactory]]`.stop(ref)`, or send a [[pekko.actor.PoisonPill]],
 * for this purpose.
 *
 ...
 */
private[pekko] trait ActorRefScope {
  def isLocal: Boolean
}
 ...
 */
private[pekko] trait LocalRef extends ActorRefScope {
 ...
 */
private[pekko] trait RepointableRef extends ActorRefScope {
 ...
 * of invokeinterface.
 */
private[pekko] abstract class ActorRefWithCell extends InternalActorRef { this: ActorRefScope =>
  def underlying: Cell
 ...
 * INTERNAL API
 */
@SerialVersionUID(1L) private[pekko] class SerializedNobody extends Serializable {
  @throws(classOf[java.io.ObjectStreamException])
 ...
 *  INTERNAL API
 */
private[pekko] class LocalActorRef private[pekko] (
   ...
   */
  @InternalApi
  override private[pekko] def isTerminated: Boolean = actorCell.isTerminated
  ...
  def readResolve(): AnyRef = currentSystem.value match {
    case null =>
      throw new IllegalStateException(
        ...
        " Use 'org.apache.pekko.serialization.JavaSerializer.currentSystem.withValue(system) { ... }'")
    case someSystem =>
 ...
 * INTERNAL API
 */
private[pekko] trait MinimalActorRef extends InternalActorRef with LocalRef {
  ...
  override private[pekko] def isTerminated = false
 ...
 * INTERNAL API
 */
@InternalApi private[pekko] final class IgnoreActorRef(
    override val provider: ActorRefProvider) extends MinimalActorRef {
 ...
 * INTERNAL API
 */
@InternalApi private[pekko] object IgnoreActorRef {
 ...
 * INTERNAL API
 */
@InternalApi @SerialVersionUID(1L) private[pekko] object SerializedIgnore extends Serializable {
  @throws(classOf[java.io.ObjectStreamException])
...
private[pekko] object DeadLetterActorRef {
  @SerialVersionUID(1L)
 ...
 * INTERNAL API
 */
private[pekko] class EmptyLocalActorRef(
    override val provider: ActorRefProvider,
    override val path: ActorPath,
    val eventStream: EventStream)
    extends MinimalActorRef {

  override private[pekko] def isTerminated = true
 ...
 * received message to the eventStream, wrapped as [[pekko.actor.DeadLetter]].
 *
 * INTERNAL API
 */
private[pekko] class DeadLetterActorRef(_provider: ActorRefProvider, _path: ActorPath, _eventStream: EventStream)
 ...
 * INTERNAL API
 */
private[pekko] class VirtualPathContainer(
    override val provider: ActorRefProvider,
    override val path: ActorPath,
 ...
 * INTERNAL API
 */
@InternalApi private[pekko] object FunctionRef {
 ...
 */
@InternalApi private[pekko] final class FunctionRef(
    override val path: ActorPath,
    override val provider: ActorRefProvider,
    system: ActorSystem,
    f: (ActorRef, Any) => Unit)
   ...
   */
  @InternalApi
  override private[pekko] def isTerminated: Boolean = _watchedBy.isEmpty
