...
abstract class ActorSelection extends Serializable {
  ...
  protected[pekko] val anchor: ActorRef
   ...
   * The output is similar to the URI fragment returned by [[pekko.actor.ActorPath#toSerializationFormat]].
   * @return URI fragment
   */
...
object ActorSelection {
   ...
   */
  private[pekko] def deliverSelection(anchor: InternalActorRef, sender: ActorRef, sel: ActorSelectionMessage): Unit =
 ...
 */
@SerialVersionUID(2L)
private[pekko] final case class SelectChildName(name: String) extends SelectionPathElement {
 ...
 */
@SerialVersionUID(2L)
private[pekko] case object SelectParent extends SelectionPathElement {
  override def toString: String = ".."
}
