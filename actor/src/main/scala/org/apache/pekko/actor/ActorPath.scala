...
final case class RootActorPath(address: Address, name: String = "/") extends ActorPath {
   ...
   * INTERNAL API
   */
  private[pekko] def uid: Int = ActorCell.undefinedUid

  /**
   * INTERNAL API
   */
  override private[pekko] def withUid(uid: Int): ActorPath =
...
@SerialVersionUID(1L)
final class ChildActorPath private[pekko] (val parent: ActorPath, val name: String,
    override private[pekko] val uid: Int)
    extends ActorPath {
  if (name.indexOf('/') != -1)
  ...
  override def hashCode: Int = {
    import pekko.routing.MurmurHash._
