 ...
 */
@SerialVersionUID(1L)
final case class Address private[pekko] (protocol: String, system: String, host: Option[String], port: Option[Int]) {
   ...
   * hosts. See also [[pekko.actor.Address#hasGlobalScope]].
   */
  def hasLocalScope: Boolean = host.isEmpty

  /**
   * Returns true if this Address is usable globally. Unlike locally defined addresses ([[org.apache.pekko.actor.Address#hasLocalScope]])
   ...
   */
  @InternalApi
  private[pekko] def hasInvalidHostCharacters: Boolean =
    host.exists(Address.InvalidHostRegex.findFirstIn(_).nonEmpty)

  /** INTERNAL API */
  @InternalApi
  private[pekko] def checkHostCharacters(): Unit =
