 ...
 *  for your marker.
 *
 *  For a `Stash` that also enforces unboundedness of the deque see [[pekko.actor.UnboundedStash]]. For a `Stash`
 *  that does not enforce any mailbox type see [[pekko.actor.UnrestrictedStash]].
 *
...
/**
 * A version of [[pekko.actor.Stash]] that does not enforce any mailbox type. The proper mailbox has to be configured
 ...
 * @see [[StashSupport]]
 */
private[pekko] trait StashFactory { this: Actor =>
  private[pekko] def createStash()(implicit ctx: ActorContext, ref: ActorRef): StashSupport = new StashSupport {
 ...
 */
private[pekko] trait StashSupport {
   ...
   */
  private[pekko] def context: ActorContext
   ...
   */
  private[pekko] def self: ActorRef
