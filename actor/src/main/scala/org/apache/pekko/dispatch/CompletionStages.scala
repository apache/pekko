...
object CompletionStages {
  ...
  def sequence[T](
    ...
    while (iterator.hasNext) {
      val r = iterator.next()
      result = if (executor eq null) {
        result.thenCombine(r, addToListFunction())
      } else {
        result.thenCombineAsync(r, addToListFunction(), executor)
      }
    }
    result
  }
  ...
  def traverse[T, R](
    ...
    while (iterator.hasNext) {
      val r = function.apply(iterator.next())
      result = if (executor eq null) {
        result.thenCombine(r, addToListFunction())
      } else {
        result.thenCombineAsync(r, addToListFunction(), executor)
