 ...
 * INTERNAL API
 *
 * Helper companion object for [[pekko.dispatch.sysmsg.LatestFirstSystemMessageList]] and
 * [[pekko.dispatch.sysmsg.EarliestFirstSystemMessageList]]
 */
private[pekko] object SystemMessageList {
 ...
 * is hidden, and can only accessed through the value classes [[pekko.dispatch.sysmsg.LatestFirstSystemMessageList]] and
 * [[pekko.dispatch.sysmsg.EarliestFirstSystemMessageList]], abstracting over the fact that system messages are the
 ...
 */
private[pekko] class LatestFirstSystemMessageList(val head: SystemMessage) extends AnyVal {
   ...
   * should be taken when passing the tail to other methods. [[pekko.dispatch.sysmsg.SystemMessage#unlink]] should be
 ...
 * latest appended element.
 */
private[pekko] class EarliestFirstSystemMessageList(val head: SystemMessage) extends AnyVal {
  import SystemMessageList._
   ...
   * number of elements.
   *
   * The type of the returned list is of the opposite order: [[pekko.dispatch.sysmsg.LatestFirstSystemMessageList]]
   */
 ...
 * INTERNAL API
 */
private[pekko] trait StashWhenWaitingForChildren
 ...
 */
@SerialVersionUID(1L)
private[pekko] final case class Create(failure: Option[ActorInitializationException]) extends SystemMessage // sent to self from Dispatcher.register
/**
 * INTERNAL API
 */
@SerialVersionUID(1L)
private[pekko] final case class Recreate(cause: Throwable) extends SystemMessage with StashWhenWaitingForChildren // sent to self from ActorCell.restart
/**
 * INTERNAL API
 */
@SerialVersionUID(1L)
private[pekko] final case class Suspend() extends SystemMessage with StashWhenWaitingForChildren // sent to self from ActorCell.suspend
/**
 * INTERNAL API
 */
@SerialVersionUID(1L)
private[pekko] final case class Resume(causedByFailure: Throwable) extends SystemMessage
    with StashWhenWaitingForChildren // sent to self from ActorCell.resume
/**
 * INTERNAL API
 */
@SerialVersionUID(1L)
private[pekko] final case class Terminate() extends SystemMessage with DeadLetterSuppression // sent to self from ActorCell.stop

/**
 * INTERNAL API
 */
@SerialVersionUID(1L)
private[pekko] final case class Supervise(child: ActorRef, async: Boolean) extends SystemMessage // sent to supervisor ActorRef from ActorCell.start
/**
 * INTERNAL API
 */
@SerialVersionUID(1L)
private[pekko] final case class Watch(watchee: InternalActorRef, watcher: InternalActorRef) extends SystemMessage // sent to establish a DeathWatch
/**
 * INTERNAL API
 */
...
private[pekko] final case class Unwatch(watchee: ActorRef, watcher: ActorRef) extends SystemMessage // sent to tear down a DeathWatch
/**
 * INTERNAL API
 */
@SerialVersionUID(1L)
private[pekko] case object NoMessage extends SystemMessage // switched into the mailbox to signal termination

/**
 * INTERNAL API
 */
@SerialVersionUID(1L)
private[pekko] final case class Failed(child: ActorRef, cause: Throwable, uid: Int)
    extends SystemMessage
    with StashWhenFailed
    with StashWhenWaitingForChildren

@SerialVersionUID(1L)
private[pekko] final case class DeathWatchNotification(
    actor: ActorRef,
