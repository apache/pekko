 ...
 * works for Lambdas that are Serializable. The latter restriction is due to the
    ...
    def readOne(d: DataInputStream): Unit =
      (d.readByte(): @switch) match {
        ...
        case 20 => // Package (JVMS 4.4.12, class file format 53.0 / Java 9)
          ...
          nextIdx += 1
        case other =>
          // An unknown tag means the rest of the pool cannot be located, since entry sizes are
          // tag dependent. Fail with a description rather than a MatchError; getInfo turns this
          // into an UnknownSourceFormat result.
          throw new UnsupportedOperationException(s"unknown constant pool tag [$other]")
      }
