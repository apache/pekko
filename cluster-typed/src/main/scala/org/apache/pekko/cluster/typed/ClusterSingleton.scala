 ...
 */
@InternalApi
private[pekko] object ClusterSingletonImpl {
...
abstract class ClusterSingleton extends Extension {
   ...
   * If there already is a proxy running for the given `singletonName` on this node, a [[pekko.actor.typed.ActorRef]] to that is returned.
   *
