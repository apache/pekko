...
class TakeLastSinkSpec
  ...
  "Sink.takeLast" must {
    "return the last 3 elements" in {
      val input = 1 to 6
      val future: Future[Seq[Int]] = Source(input).runWith(Sink.takeLast(3))
    ...
    "return the number of elements taken when the stream completes" in {
      val input = 1 to 4
      val future: Future[Seq[Int]] = Source(input).runWith(Sink.takeLast(5))
      val result: Seq[Int] = Await.result(future, remainingOrDefault)
    ...
    "yield empty seq for empty stream" in {
      val future: Future[Seq[Int]] = Source.empty[Int].runWith(Sink.takeLast(3))
      val result: Seq[Int] = Await.result(future, remainingOrDefault)
    ...
    "return the last element" in {
      val input = 1 to 4
      val future: Future[Seq[Int]] = Source(input).runWith(Sink.takeLast(1))
      val result: Seq[Int] = Await.result(future, remainingOrDefault)
