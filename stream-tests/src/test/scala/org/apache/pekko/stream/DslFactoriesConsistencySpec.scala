...
class DslFactoriesConsistencySpec extends AnyWordSpec with Matchers {
  ...
  val `scala -> java types` =
    (classOf[scala.collection.immutable.Iterable[_]],      classOf[java.lang.Iterable[_]]) ::
      ...
      (classOf[scala.Option[_]],                           classOf[java.util.Optional[_]]) ::
      (classOf[Boolean],                                   classOf[pekko.stream.javadsl.AsPublisher]) ::
  ...
  private def ignore(m: Method): Boolean = {
    val ignores = Seq(
      ...
      Ignore(_ == pekko.stream.scaladsl.Sink.getClass, _ == "apply", _ == 24, _ => true),
      Ignore(_ == pekko.stream.scaladsl.Sink.getClass, _ == "collection", _ => true, _ => true),
      Ignore(_ == pekko.stream.scaladsl.Sink.getClass, _ == "actorRef", _ => true, _ => true), // Internal in scaladsl
      Ignore(_ == pekko.stream.scaladsl.Sink.getClass, _ == "actorRefWithAck", _ => true, _ => true), // Internal in scaladsl
      Ignore(_ == pekko.stream.scaladsl.Sink.getClass, _ == "actorRefWithBackpressure", _ => true, _ => true), // Internal in scaladsl
