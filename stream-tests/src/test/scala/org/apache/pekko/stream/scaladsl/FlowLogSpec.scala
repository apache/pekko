...
import pekko.event.{ DummyClassForStringSources, Logging }
...
class FlowLogSpec extends StreamSpec("""
  ...
  "A Log" must {
    ...
    val LogSrc = s"org.apache.pekko.stream.Log($supervisorPath)"
    ...
    "on javadsl.Flow" must {
      "debug each element" in {
        ...
        val debugging: javadsl.Flow[Integer, Integer, NotUsed] = javadsl.Flow
          .of(classOf[Integer])
          .log("log-1")
          .log("log-2", new pekko.japi.function.Function[Integer, Integer] { def apply(i: Integer) = i })
          .log("log-3", new pekko.japi.function.Function[Integer, Integer] { def apply(i: Integer) = i }, log)
          .log("log-4", log)
