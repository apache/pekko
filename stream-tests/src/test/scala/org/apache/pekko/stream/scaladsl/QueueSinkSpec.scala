...
class QueueSinkSpec extends StreamSpec {
  ...
  "An QueueSinkSpec" must {
    ...
    "keep on sending even after the buffer has been full" in {
      ...
      val bufferFullProbe = Promise[pekko.Done.type]()
      val queue = Source(1 to streamElementCount)
        .alsoTo(Flow[Int].drop(bufferSize - 1).to(Sink.foreach(_ => bufferFullProbe.trySuccess(pekko.Done))))
        .toMat(sink)(Keep.right)
        .run()
      bufferFullProbe.future.futureValue should ===(pekko.Done)
    ...
    "materialize to a queue which is seamlessly translatable between scala and java DSL" in {
      ...
      val scaladslQueue = pekko.stream.javadsl.SinkQueueWithCancel.asScala(javadslQueue)
      expected.foreach { v =>
