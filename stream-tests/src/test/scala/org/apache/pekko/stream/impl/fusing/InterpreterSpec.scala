...
class InterpreterSpec extends StreamSpec with GraphInterpreterSpecKit {
  ...
  @nowarn
  private[pekko] final case class Doubler[T]() extends SimpleLinearGraphStage[T] {
  ...
  @nowarn
  private[pekko] final case class KeepGoing[T]() extends SimpleLinearGraphStage[T] {
  ...
  private[pekko] class PushFinishStage(onPostStop: () => Unit = () => ()) extends SimpleLinearGraphStage[Any] {
    override def createLogic(inheritedAttributes: Attributes): GraphStageLogic =
      new GraphStageLogic(shape) with InHandler with OutHandler {
        ...
        override def onPull(): Unit = pull(in)
        override def onUpstreamFinish(): Unit = failStage(pekko.stream.testkit.Utils.TE("Cannot happen"))
