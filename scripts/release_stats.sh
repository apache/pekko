...
declare open_milestones=$(curl -s -H "$script_user_agent" "https://api.github.com/repos/apache/pekko/milestones?state=open")
declare closed_milestones=$(curl -s -H "$script_user_agent" "https://api.github.com/repos/apache/pekko/milestones?state=closed")
...
declare tickets=$(curl -s -H "$script_user_agent" "https://api.github.com/repos/apache/pekko/issues?milestone=$milestone_id&state=all&per_page=100" | sed 's/"comments"/\n/g' | perl -ne 'm/number":([0-9]+),"title":"(.+?)",/ && print " - *$1* $2\n"' | sort -n)
