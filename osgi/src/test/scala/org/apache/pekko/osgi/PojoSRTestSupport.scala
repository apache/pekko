...
trait PojoSRTestSupport extends Suite with BeforeAndAfterAll {
  ...
  lazy val context: BundleContext = {
    ...
    System.setProperty("org.osgi.framework.storage", "target/osgi/" + UUID.randomUUID().toString)
  ...
  def filterErrors()(block: => Unit): Unit =
    try block
    catch { case e: Throwable => System.err.write(bufferedLoadingErrors.toByteArray); throw e }
}
