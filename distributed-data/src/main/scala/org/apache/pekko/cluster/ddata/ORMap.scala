...
object ORMap {
   ...
   */
  @InternalApi private[pekko] trait ZeroTag {
   ...
   * INTERNAL API
   */
  @InternalApi private[pekko] case object VanillaORMapTag extends ZeroTag {
   ...
   * INTERNAL API
   */
  @InternalApi private[pekko] sealed abstract class AtomicDeltaOp[A, B <: ReplicatedData]
      extends DeltaOp
  ...
  /** INTERNAL API */
  @InternalApi private[pekko] final case class PutDeltaOp[A, B <: ReplicatedData](
  ...
  /** INTERNAL API */
  @InternalApi private[pekko] final case class UpdateDeltaOp[A, B <: ReplicatedData](
      underlying: ORSet.DeltaOp,
      values: Map[A, B],
      zeroTag: ZeroTag)
      extends AtomicDeltaOp[A, B] {
  ...
  /** INTERNAL API */
  @InternalApi private[pekko] final case class RemoveDeltaOp[A, B <: ReplicatedData](
      underlying: ORSet.DeltaOp,
      zeroTag: ZeroTag)
      extends AtomicDeltaOp[A, B]
  ...
  /** INTERNAL API */
  @InternalApi private[pekko] final case class RemoveKeyDeltaOp[A, B <: ReplicatedData](
      underlying: ORSet.DeltaOp,
      removedKey: A,
      zeroTag: ZeroTag)
      extends AtomicDeltaOp[A, B]
  ...
  /** INTERNAL API */
  @InternalApi private[pekko] final case class DeltaGroup[A, B <: ReplicatedData](ops: immutable.IndexedSeq[DeltaOp])
      extends DeltaOp
      with ReplicatedDeltaSize {
    override def merge(that: DeltaOp): DeltaOp = that match {
 ...
 */
@SerialVersionUID(1L)
final class ORMap[A, B <: ReplicatedData] private[pekko] (
    private[pekko] val keys: ORSet[A],
    private[pekko] val values: Map[A, B],
    private[pekko] val zeroTag: ZeroTag,
   ...
   * INTERNAL API
   */
  @InternalApi private[pekko] def updated(node: UniqueAddress, key: A, initial: B, valueDeltas: Boolean = false)(
   ...
   * INTERNAL API
   */
  @InternalApi private[pekko] def remove(node: UniqueAddress, key: A): ORMap[A, B] = {
   ...
   */
  @InternalApi private[pekko] def removeKey(node: UniqueAddress, key: A): ORMap[A, B] = {
    val newKeys = keys.resetDelta.remove(node, key)
   ...
   * by keeping the vvector (in form of key -> value pair) for deleted keys
   */
  @InternalApi private[pekko] def mergeRetainingDeletedValues(that: ORMap[A, B]): ORMap[A, B] = {
    val mergedKeys = keys.merge(that.keys)
   ...
   * by keeping the vvector (in form of key -> value pair) for deleted keys
   */
  @InternalApi private[pekko] def mergeDeltaRetainingDeletedValues(thatDelta: ORMap.DeltaOp): ORMap[A, B] = {
