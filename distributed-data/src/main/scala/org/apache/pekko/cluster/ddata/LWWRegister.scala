...
object LWWRegister {
   ...
   * INTERNAL API
   */
  @InternalApi private[pekko] def apply[A](node: UniqueAddress, initialValue: A, clock: Clock[A]): LWWRegister[A] =
 ...
 */
@SerialVersionUID(1L)
final class LWWRegister[A] private[pekko] (private[pekko] val node: UniqueAddress, val value: A, val timestamp: Long)
   ...
   * INTERNAL API
   */
  @InternalApi private[pekko] def withValue(node: UniqueAddress, value: A, clock: Clock[A]): LWWRegister[A] =
