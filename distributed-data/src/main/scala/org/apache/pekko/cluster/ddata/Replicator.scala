...
object ReplicatorSettings {
   ...
   * INTERNAL API
   */
  @InternalApi private[pekko] def parseExpiry(config: Config): Map[KeyId, FiniteDuration] = {
    import scala.jdk.CollectionConverters._
   ...
   * INTERNAL API
   */
  @InternalApi private[pekko] def roleOption(role: String): Option[String] =
...
object Replicator {
   ...
   * INTERNAL API
   */
  @InternalApi private[pekko] case object GetKeyIds

  /**
   * INTERNAL API
   */
  @InternalApi private[pekko] final case class GetKeyIdsResult(keyIds: Set[KeyId]) {
   ...
   * INTERNAL API
   */
  @InternalApi private[pekko] object Internal {
...
final class Replicator(settings: ReplicatorSettings) extends Actor with ActorLogging {
  ...
  def setData(key: KeyId, envelope: DataEnvelope): DataEnvelope = {
    ...
    dataEntries = dataEntries.updated(key, (newEnvelope, dig, getUsedTimestamp(key)))
  ...
  private def createGossipMessages(
    ...
    val collectedEntries = Vector.newBuilder[(KeyId, (DataEnvelope, Timestamp))]
    var sum = 0
 ...
 * INTERNAL API
 */
@InternalApi private[pekko] object ReadWriteAggregator {
 ...
 * INTERNAL API
 */
@InternalApi private[pekko] abstract class ReadWriteAggregator extends Actor {
 ...
 * INTERNAL API
 */
@InternalApi private[pekko] object WriteAggregator {
  def props(
      key: KeyR,
 ...
 * INTERNAL API
 */
@InternalApi private[pekko] class WriteAggregator(
    key: KeyR,
    envelope: Replicator.Internal.DataEnvelope,
    delta: Option[Replicator.Internal.Delta],
    consistency: Replicator.WriteConsistency,
 ...
 * INTERNAL API
 */
@InternalApi private[pekko] object ReadAggregator {
  def props(
      key: KeyR,
 ...
 * INTERNAL API
 */
@InternalApi private[pekko] class ReadAggregator(
    key: KeyR,
    consistency: Replicator.ReadConsistency,
    req: Option[Any],
    selfUniqueAddress: UniqueAddress,
