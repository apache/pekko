...
object VersionVector {
  ...
  /** INTERNAL API */
  @InternalApi private[pekko] def apply(versions: List[(UniqueAddress, Long)]): VersionVector =
    if (versions.isEmpty) empty
  ...
  /** INTERNAL API */
  @InternalApi private[pekko] object Timestamp {
    final val Zero = 0L
...
sealed abstract class VersionVector extends ReplicatedData with ReplicatedDataSerialization with RemovedNodePruning {
   ...
   * Increment the version for the node passed as argument. Returns a new VersionVector.
   */
  @InternalApi private[pekko] def +(node: UniqueAddress): VersionVector = increment(node)
   ...
   * INTERNAL API
   */
  @InternalApi private[pekko] def size: Int

  /**
   * INTERNAL API
   * Increment the version for the node passed as argument. Returns a new VersionVector.
   */
  @InternalApi private[pekko] def increment(node: UniqueAddress): VersionVector

  /**
   * INTERNAL API
   */
  @InternalApi private[pekko] def versionAt(node: UniqueAddress): Long

  /**
   * INTERNAL API
   */
  @InternalApi private[pekko] def contains(node: UniqueAddress): Boolean
   ...
   * INTERNAL API
   */
  @InternalApi private[pekko] def versionsIterator: Iterator[(UniqueAddress, Long)]
  ...
  /** INTERNAL API */
  @InternalApi private[pekko] def estimatedSize: Int

}

final case class OneVersionVector private[pekko] (node: UniqueAddress, version: Long) extends VersionVector {
  import VersionVector.Timestamp

  override def isEmpty: Boolean = false

  /** INTERNAL API */
  @InternalApi private[pekko] override def size: Int = 1

  /** INTERNAL API */
  @InternalApi private[pekko] override def increment(n: UniqueAddress): VersionVector = {
  ...
  /** INTERNAL API */
  @InternalApi private[pekko] override def versionAt(n: UniqueAddress): Long =
    if (n == node) version
    else Timestamp.Zero

  /** INTERNAL API */
  @InternalApi private[pekko] override def contains(n: UniqueAddress): Boolean =
    n == node
...
final case class ManyVersionVector(versions: TreeMap[UniqueAddress, Long]) extends VersionVector {
  ...
  /** INTERNAL API */
  @InternalApi private[pekko] override def size: Int = versions.size

  /** INTERNAL API */
  @InternalApi private[pekko] override def increment(node: UniqueAddress): VersionVector = {
    val v = Timestamp.counter.getAndIncrement()
    VersionVector(versions.updated(node, v))
  }

  /** INTERNAL API */
  @InternalApi private[pekko] override def versionAt(node: UniqueAddress): Long = versions.get(node) match {
    case Some(v) => v
    case None    => Timestamp.Zero
  }

  /** INTERNAL API */
  @InternalApi private[pekko] override def contains(node: UniqueAddress): Boolean =
