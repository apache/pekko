...
class Cluster(val system: ExtendedActorSystem) extends Extension {
  ...
  private def checkAutoDownUsage(): Unit = {
    if (settings.DowningProviderClassName == "org.apache.pekko.cluster.AutoDowning" ||
      ...
      logWarning(
        ...
        "https://pekko.apache.org/docs/pekko/2.6/typed/cluster.html#downing for alternatives.")
  }
   ...
   * The `to` classes can be [[pekko.cluster.ClusterEvent.ClusterDomainEvent]]
   * or subclasses.
   *
   ...
   * [[pekko.cluster.ClusterEvent.CurrentClusterState]] will be sent to the subscriber as the
   * first message.
   *
  ...
  @varargs def subscribe(subscriber: ActorRef, initialStateMode: SubscriptionInitialStateMode, to: Class[_]*): Unit = {
    ...
    require(
      ...
      s"subscribe to `org.apache.pekko.cluster.ClusterEvent.ClusterDomainEvent` or subclasses, was [${to.map(
          _.getName).mkString(", ")}]")
   ...
   *
   * The name of the [[pekko.actor.ActorSystem]] must be the same for all members of a
   * cluster.
   */
