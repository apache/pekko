...
class ClusterMessageSerializerSpec extends PekkoSpec("pekko.actor.provider = cluster") {
  ...
  "ClusterMessages" must {
    ...
    "reject the class-based manifests that Akka 2.6.4 and earlier nodes sent" in {
      // these were kept "for one version iteration from Akka 2.6.4" to allow rolling
      // migration to the short manifests; nothing a Pekko node can talk to emits them,
      // so they must be refused as unknown rather than kept as wire-reachable paths
      val address = Address("pekko", "system", "some.host.org", 4711)
      ...
      Seq(
        ...
        "org.apache.pekko.cluster.routing.ClusterRouterPool").foreach { manifest =>
