...
class ManifestInfoSpec extends AkkaSpec {
  "ManifestInfo" should {
    "produce a clear message" in {
      ...
      val allModules = List("pekko-actor", "akka-persistence", "akka-cluster")
      ManifestInfo.checkSameVersion("Akka", allModules, versions) shouldBe Some(
        ...
        "Here's a complete collection of detected artifacts: (2.5.3, [akka-cluster, akka-persistence]), (2.6.4, [pekko-actor]). " +
    ...
    "support dynver" in {
      ...
      val allModules = List("pekko-actor", "akka-persistence")
      ManifestInfo.checkSameVersion("Akka", allModules, versions) shouldBe Some(
        ...
        "Here's a complete collection of detected artifacts: (2.6.4, [pekko-actor]), (2.6.4+10-abababef, [akka-persistence]). " +
        "See also: https://doc.akka.io/docs/akka/current/common/binary-compatibility-rules.html#mixed-versioning-is-not-allowed")
    }
  }
}
