...
trait SubchannelClassification { this: EventBus =>
   ...
   */
  private[pekko] def hasSubscriptions(subscriber: Subscriber): Boolean =
...
trait ManagedActorClassification { this: ActorEventBus with ActorClassifier =>
   ...
   * INTERNAL API
   */
  private[pekko] def registerWithUnsubscriber(subscriber: ActorRef, seqNr: Int): Boolean = {
   ...
   * INTERNAL API
   */
  private[pekko] def unregisterFromUnsubscriber(subscriber: ActorRef, seqNr: Int): Boolean = {
