...
import pekko.japi.{ Creator, Option => JOption }
import pekko.japi.Util.{ immutableSeq, immutableSingletonSeq }
import pekko.pattern.AskTimeoutException
import pekko.serialization.{ JavaSerializer, SerializationExtension, Serializers }
import pekko.util.Reflect.instantiator
...
trait TypedActorFactory {
  ...
  def typedActorOf[R <: AnyRef, T <: R](props: TypedProps[T], name: String): R = {
    ...
    val c = props.creator // Cache this to avoid closing over the Props
    val i = props.interfaces // Cache this to avoid closing over the Props
    val ap = Props(new pekko.actor.TypedActor.TypedActor[R, T](proxyVar, c(), i)).withDeploy(props.actorProps().deploy)
    typedActor.createActorRefProxy(props, proxyVar, actorFactory.actorOf(ap, name))
  }

  /**
   * Creates a TypedActor that intercepts the calls and forwards them as [[pekko.actor.TypedActor.MethodCall]]
...
object TypedActor extends ExtensionId[TypedActorExtension] with ExtensionIdProvider {
  ...
  final case class MethodCall(method: Method, parameters: Array[AnyRef]) {
    ...
    @throws(classOf[ObjectStreamException]) private def writeReplace(): AnyRef = parameters match {
      ...
      case ps =>
        val serialization = SerializationExtension(pekko.serialization.JavaSerializer.currentSystem.value)
  ...
  private[pekko] final case class SerializedMethodCall(
    ...
    @throws(classOf[ObjectStreamException]) private def readResolve(): AnyRef = {
      val system = pekko.serialization.JavaSerializer.currentSystem.value
      if (system eq null)
  ...
  private[pekko] class TypedActor[R <: AnyRef, T <: R](
    ...
    protected def withContext[U](unitOfWork: => U): U = {
      pekko.actor.TypedActor.selfReference.set(proxyVar.get)
      pekko.actor.TypedActor.currentContext.set(self.context)
      try unitOfWork
      finally {
        pekko.actor.TypedActor.selfReference.set(null)
        pekko.actor.TypedActor.currentContext.set(null)
      }
    }
   ...
   * INTERNAL API
   */
  private[pekko] final case class SerializedTypedActorInvocationHandler(
      val actor: ActorRef,
...
class TypedActorExtension(val system: ExtendedActorSystem) extends TypedActorFactory with Extension {
   ...
   * INTERNAL API
   */
  private[pekko] def createActorRefProxy[R <: AnyRef, T <: R](
      props: TypedProps[T],
      proxyVar: AtomVar[R],
      actorRef: => ActorRef): R = {
