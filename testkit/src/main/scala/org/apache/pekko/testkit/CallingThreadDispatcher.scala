...
private[testkit] class CallingThreadDispatcherQueues extends Extension {
  ...
  protected[pekko] def unregisterQueues(mbox: CallingThreadMailbox): Unit = synchronized {
    queues -= mbox
  }
   ...
   * (active).
   */
  protected[pekko] def gatherFromAllOtherQueues(mbox: CallingThreadMailbox, own: MessageQueue): Unit = synchronized {
    if (queues contains mbox) {
      for {
        ref <- queues(mbox)
        q = ref.get
...
class CallingThreadDispatcher(_configurator: MessageDispatcherConfigurator) extends MessageDispatcher(_configurator) {
  ...
  val log = pekko.event.Logging(eventStream, classOf[CallingThreadDispatcher])

  override def id: String = Id

  protected[pekko] override def createMailbox(actor: pekko.actor.Cell, mailboxType: MailboxType) =
    new CallingThreadMailbox(actor, mailboxType)

  protected[pekko] override def shutdown(): Unit = {}

  protected[pekko] override def throughput = 0
  protected[pekko] override def throughputDeadlineTime = Duration.Zero
...
class CallingThreadMailbox(_receiver: pekko.actor.Cell, val mailboxType: MailboxType)
